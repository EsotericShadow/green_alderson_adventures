...
func get_fireball() -> Node:
	...
	# Reset Area2D monitoring to clear any stale collision state
	if fireball is Area2D:
		fireball.set_deferred("monitoring", false)
...
func _deferred_return_fireball(fireball: Node) -> void:
	if fireball == null:
		return
	...
	# Disable monitoring to prevent phantom collisions
	if fireball is Area2D:
...
func get_impact() -> Node:
	...
	# Re-enable processing for animation
	impact.set_process(true)
	impact.set_physics_process(true)
	impact.visible = true
	
	# Make sure AnimatedSprite2D is also enabled
	var sprite = impact.get_node_or_null("AnimatedSprite2D")
	if sprite != null:
		sprite.visible = true
...
func return_impact(impact: Node) -> void:
	...
	# Use call_deferred to avoid physics callback issues
	call_deferred("_deferred_return_impact", impact)


func _deferred_return_impact(impact: Node) -> void:
	if impact == null:
		return

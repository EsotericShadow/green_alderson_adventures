...
## Provides consistent interface and initialization pattern.
...
func update(delta: float) -> void:
	"""Updates the worker. Override _on_update() in subclasses.
...
func _on_update(_delta: float) -> void:
	"""Called each frame. Override for per-frame updates.
